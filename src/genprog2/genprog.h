...
public:
    ...
    char* projection_details;
    char* default_ratio;
    char* ratio_threshold;
    char* optimize_code;
    char* fixed_random_seed;
    char* n_cores;
    ...
    void do_adjustments (int&);
    void assess_compute_need ();
    void get_vars_in (const char*, const int, int*&, int&);
};

bool is_random (const char* var);
char* threaded_val (const char*);
void floatise (char*&);
